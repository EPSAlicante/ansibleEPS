...
def execOption(opt):
	...
	print >> sys.stderr
      ...
      ## Read Modules Help ##
      # Modules: common, hostsFile, sudo, wrappers, pamAccess, nrpe, iptables, proxmox, mycnf, apacheInc, winNrpe, bacula, dhcp, munin, nagios, emailList, update, cron
      try:
	print
	module = raw_input('Module (common, hostsFile, sudo, wrappers, pamAccess, nrpe, iptables, proxmox, mycnf, apacheInc, winNrpe, bacula, dhcp, munin, nagios, emailList, update, cron): ')
	if module.lower() in ['common', 'hostsfile', 'sudo', 'wrappers', 'pamaccess', 'nrpe', 'iptables', 'proxmox', 'mycnf', 'apacheinc', 'winnrpe', 'bacula', 'dhcp', 'munin', 'nagios', 'emaillist', 'update', 'cron']:
	  pathHelpModule = "%s/help/%s" % (pathAnsible, module.upper())
          ...
          print >> sys.stderr
          print >> sys.stderr, "Module %s not valid (Valid modules: common, hostsFile, sudo, wrappers, pamAccess, nrpe, iptables, proxmox, mycnf, apacheInc, winNrpe, bacula, dhcp, munin, nagios, emailList, update, cron)" % (module)
          print >> sys.stderr
