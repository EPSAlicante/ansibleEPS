#!/bin/sh
#
# Event handler script for starting Iptables.sh service 
#
...
CRITICAL)
	...
	SOFT)
		...
		case "$3" in
			...
			3)
				echo -n "Starting Iptables.sh service in $4 (3rd soft critical state)..."							
				# Call the init script to start the server
				/usr/lib/nagios/plugins/check_nrpe -H "$4" -c restart-iptables.sh				
				;;
		esac
		;;
	...
	HARD)
		echo -n "Starting Iptables service in $4..."
